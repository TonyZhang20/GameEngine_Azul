...
inline map_index_t Hash(absl::string_view k, void* salt) {
  // Note: we could potentially also use CRC32-based hashing here.
    // Keenan map_index_t
  return (map_index_t)absl::HashOf(k, salt);
}
inline map_index_t Hash(uint64_t k, void* salt) {
    // Keenan map_index_t
  if constexpr (!HasCrc32()) return (map_index_t)absl::HashOf(k, salt);
  ...
  // we also rotate.
  // Keenan map_index_t uint32_t this is x64, their code is wrong
  return (map_index_t) Crc32((uint32_t)salt_int, absl::rotr(k, salt_int & 0x3f));
}
