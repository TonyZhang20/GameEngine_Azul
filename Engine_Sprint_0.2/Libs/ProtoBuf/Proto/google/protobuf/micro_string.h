...
class PROTOBUF_EXPORT MicroString {
  struct LargeRep {
    ...
    char* owned_head() {
        // Keenan uint32_t
      ABSL_DCHECK_GE(capacity, (uint32_t)kOwned);
    ...
    void SetInitialSize(size_t s) {
      PoisonMemoryRegion(owned_head() + s, capacity - s);
      // Keenan uint32_t
      this->size = (uint32_t) s;
    }
  ...
  struct MicroRep {
    ...
    void SetInitialSize(uint8_t s) {
      PoisonMemoryRegion(data() + s, capacity - s);
      this->size = s;
    }
